...
public class RegistroPublicacionesAutor {
    ...
    public double calcularIndiceC() {
        ...
        for (Publicacion publicacion : publicaciones) {
            if (publicacion instanceof Articulo) {
                numArt += publicacion.getCitas();
            } else if (publicacion instanceof Libro) {
                numLib += publicacion.getCitas();
            } else if (publicacion instanceof Comunicacion) {
                numCom += publicacion.getCitas();
            }
        }

        return (numArt + (numLib * 0.8) + (numCom * 0.6));
    }
